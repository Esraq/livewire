...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Syndicate</h1>
    ...
    <div class="card mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Syndicate Members</h6>
        </div>
            ...
            <table class="table table-hover align-items-center mb-0">
                ...
                @forelse ($syndicates as $syndicate)
                    <tr>
                        ...
                        <td>{{ $syndicate->weight }}</td>

                        <td>
                            <img src="{{ $syndicate->image_url }}" alt="{{ $syndicate->name }}"
                                 class="rounded" style="width:70px;height:70px;object-fit:cover;">
                        </td>

                        <td>{{ $syndicate->name }}</td>

                        <td>{{ $syndicate->position }}</td>

                        <td class="text-center">

                            <button type="button"
                                    class="btn btn-sm btn-primary edit-btn"
                                    data-action="{{ route('syndicates.update', $syndicate) }}"
                                    data-id="{{ $syndicate->id }}"
                                    data-weight="{{ $syndicate->weight }}"
                                    data-name="{{ $syndicate->name }}"
                                    data-position="{{ $syndicate->position }}"
                                    data-image="{{ $syndicate->image_url }}">
                                <i class="fas fa-edit"></i>
                            </button>

                            <form action="{{ route('syndicates.destroy', $syndicate) }}" method="POST"
                                  class="d-inline"
                ...
                @empty
                    <tr>
                        <td colspan="6" class="text-center text-muted py-4">No syndicate members found.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
...
<div class="modal fade" id="syndicateModal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">

            <form id="syndicateForm" method="POST" enctype="multipart/form-data">
                @csrf
...
$(function () {
    ...
    var storeUrl  = "{{ route('syndicates.store') }}";
    var updateUrl = "{{ route('syndicates.update', '__ID__') }}";

    var $modal   = $('#syndicateModal');
    var $form    = $('#syndicateForm');
