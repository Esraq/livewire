...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <button type="button" class="btn btn-primary" id="createBtn">
            <i class="fas fa-plus fa-sm"></i> Add Alumni
        </button>
    </div>
    ...
    <div class="card mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Alumni List</h6>
        </div>
            ...
            <table class="table table-hover align-items-center mb-0">
                ...
                @forelse ($alumnis as $alumni)
                    <tr>
                        ...
                        <td>
                            <img src="{{ $alumni->image_url }}" alt="{{ $alumni->title }}"
                                 class="rounded" style="width:80px;height:80px;object-fit:cover;">
                        </td>

                        <td>{{ $alumni->title }}</td>

                        <td>{{ \Illuminate\Support\Str::limit($alumni->description, 120) }}</td>

                        <td class="text-center">

                            <button type="button"
                                    class="btn btn-sm btn-primary edit-btn"
                                    data-action="{{ route('alumni.update', $alumni) }}"
                                    data-id="{{ $alumni->id }}"
                                    data-title="{{ $alumni->title }}"
                                    data-description="{{ $alumni->description }}"
                                    data-image="{{ $alumni->image_url }}">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="text-center text-muted py-4">No alumni found.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
...
<!-- =====================================================
     ...
     ===================================================== -->
<div class="modal fade" id="alumniModal" tabindex="-1" role="dialog" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <form id="alumniForm" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="modalTitle">Add Alumni</h5>
...
$(function () {
    ...
    var storeUrl  = "{{ route('alumni.store') }}";
    var updateUrl = "{{ route('alumni.update', '__ID__') }}";
